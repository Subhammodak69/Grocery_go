    ...
    function createPayment(paymentMethod, paymentDetails) {
        ...
        setTimeout(()=>{
            ...
            .then(data => {
                ...
                if (data.success) {
                    showMessage('success', data.message || 'Payment created successfully!', data.redirected_url);
                    loader.style.display = 'none';
                    setTimeout(()=>{
                        // Submit form directly
                    const form = document.getElementById('order-summary-form');
                    if (form) {
                        // Add payment method to form before submission
                    ...
                    showMessage('error', data.message || 'Payment creation failed', data.redirected_url);
                }
            })
            .catch(error => {
                showLoading(false);
                ...
                showMessage('error', error.message || 'An error occurred during payment');
            });
        },2000);    
    }
